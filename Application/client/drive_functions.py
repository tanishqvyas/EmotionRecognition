...
LIST_OF_FILES = drive.ListFile({'q':"'root' in parents and trashed=false"}).GetList()
...
def upload_file(title,filename,full_path='',folderID=0):
    ...
    print(f"{filename} is successfully uploaded")
...
def upload_file_save_id(log,title,filename,full_path='',folderID=0):
    ...
    try:
        log.write(str(id)+'\n')
        print(f'info about {filename} is logged')
    except:
