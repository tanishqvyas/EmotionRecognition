...
#Wait time till we get a new video in sec
WAIT_TIME = 10
...
if log_info:
    ...
    for video in range(1,log_info+1):
        video = str(video)+'.avi'
        print(video)
        emotions = face_emot_detect('data//'+video, video, '')
        ...
        try:
            upload_file_save_id(results,video,video,'',resID)
        except:
            print(f"Unable to upload {video}")
            pass
    results.close()
    ...
    while(True):
        ...
        while(not search_download(str(vid_inst)+'.avi',dataID)):
            pass
        os.chdir('..')
        ...
        results.write(f'result of {video}: and emotions found and its weight {emotions} \n')
        try:
            opt = upload_file_save_id(results,video,video,'',resID)
            if opt==False:
                error+=1
        except:
            error += 1
